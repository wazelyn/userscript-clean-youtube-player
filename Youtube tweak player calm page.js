// ==UserScript==
// @name       Youtube tweak player calm page
...
//[][][]2017-02-26-1608-32 let's also make long comments better
/*
function letCommentsOverflow()
{
    //document.getElementsByClassName("comment-renderer-text-content");
    var comments = $('div.comment-renderer-text-content');
    for (i=0;i<comments.length;i++)
        comments[i].style.overflow = "scroll";
...
tweakPlayer();
//letCommentsOverflow();
var addbuttoninterval = setTimeout (function(){
	addInvisibleToggleCommentsButton();
	//clearInterval(addbuttoninterval); //wait to add the button, then only add it once
},0); //no more conflict! savetube updated so it's in some other place now
//},1500); //[][][] conflict with savetube, where the button gets added first then savetube's bar is sloppily below it
var retweakplayer = setTimeout (function(){
	tweakPlayer();
